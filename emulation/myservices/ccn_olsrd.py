...
class CcnOlsrService(CoreService):
    ...
    def generateconfig(cls, node, filename, services):
        ...
        if filename == cls._configs[0]: # start
                return cfg + cls.generateCcnOlsrConf(node, services, ccnx_dir, olsr_dir, start=True)
        elif filename == cls._configs[1]: # stop
                return cfg + cls.generateCcnOlsrConf(node, services, ccnx_dir, olsr_dir, start=False)
...
""" % (ccnx_dir, olsr_dir, node.name, node.name, node.name)
    ...
    @classmethod
    def generateCcnOlsrConf(cls, node, services, ccnx_dir, olsr_dir, start):
            cfg = """
